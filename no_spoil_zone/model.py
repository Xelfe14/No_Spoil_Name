...
def main():
    author_name = input("Select Author (Austen, Jane / Shakespeare, William / Twain, Mark): ")
    book_name = input("Select Book (Emma / Hamlet / Tom Sawyer): ")
    ...
    try:
        ...
        if chapter_number == 1:
            print(f"🤔💭 What happens in the first chapter of {book_name}?")
        else:
            print(f"🤔💭 What happens in the first {chapter_number} chapters of {book_name}?")
        ...
        for i, (chapter_text, lexrank_summarized_text) in enumerate(zip(chapters_text, precomputed_lexrank_summaries), start=1):
            ...
            print(f"### Topics for Chapter {i}")
            print(f"**People**: {', '.join(topic_dict['persons'])}")
            print(f"**Places**: {', '.join(topic_dict['places'])}")
            print(f"**General**: {', '.join(topic_dict['general'])}")

            print(f"### Summary for Chapter {i}")
            print(response.text)
